...
- (void)drawBezelOfTabCell:(PSMTabBarCell *)cell withFrame:(NSRect)frame inTabBarControl:(PSMTabBarControl *)tabBarControl {
    ...
    if([cell state] == NSControlStateValueOn) {
        ...
        [bgImage drawInRect:cellFrame fromRect:NSMakeRect(0.0, 0.0, 1.0, 22.0) operation:NSCompositingOperationSourceOver fraction:1.0 respectFlipped:NO hints:nil];
        
        [aquaDivider drawAtPoint:NSMakePoint(cellFrame.origin.x + cellFrame.size.width - 1.0, cellFrame.origin.y) fromRect:NSZeroRect operation:NSCompositingOperationSourceOver fraction:1.0];
...
- (void)drawBezelOfTabBarControl:(PSMTabBarControl *)tabBarControl inRect:(NSRect)rect {
	if(rect.size.height <= 22.0) {
		...
		[aquaTabBg drawInRect:rect fromRect:NSMakeRect(0.0, 0.0, 1.0, 22.0) operation:NSCompositingOperationSourceOver fraction:1.0 respectFlipped:NO hints:nil];
	}
}
