...
#define SCOPE_BAR_HEIGHT				22.0																	// used in -sizeToFit
...
#define GROUP_SELECTION_MODE			@"SelectionMode"		// MGScopeBarGroupSelectionMode (NSInteger) as NSNumber.
		...
		if (numGroups > 0) {
			...
			for (NSInteger groupNum = 0; groupNum < numGroups; groupNum++) {
				...
				if (selMode != MGRadioSelectionMode && selMode != MGMultipleSelectionMode) {
					// Sanity check, since this is just an NSInteger.
	...
	*/
	...
	if (proceed) {
		...
		if (adjusting || shouldAdjustPopups) {
			...
			if (adjusting) {
				for (NSInteger i = changedRange.location; i < NSMaxRange(changedRange); i++) {
			...
			for (NSInteger i = startIndex; i < [_groups count]; i++) {
